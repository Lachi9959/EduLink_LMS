...
function App() {
  ...
  return (
    <div className="container">
      ...
      {view === "auth" && <Auth onAuth={(tok) => {
        localStorage.setItem("edulink_token", tok);
        setUser({ token: tok });
        setView("home");
      }} />}
      {view === "createCourse" && <CreateCourse onDone={() => {
        fetchCourses();
        setView("home");
      }} />}
    </div>
...
function CreateCourse({ onDone }) {
  ...
  async function submit(e) {
    ...
    try {
      await axios.post("http://localhost:5000/api/courses", f, {
        headers: { Authorization: "Bearer " + tok }
      });
